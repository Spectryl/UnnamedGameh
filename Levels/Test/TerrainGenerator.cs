...
public partial class TerrainGenerator : Node {
	...
	public static Action TerrainReady;
	...
	private async Task CreateTerrain() {
		...
		terrain3D = Terrain3D.Instantiate();
		...
		AddChild(terrain3D, true);
		terrain3D.Owner = GetTree().GetCurrentScene();
		
		var material = terrain3D.Material;
		...
		data.ImportImages(images, new Vector3(-1024, 0, -1024), 0.0, 30.0);
			
		// Setup Instancer
		var instancer = terrain3D.Instancer;
